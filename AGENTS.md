...
* External references (read-only, don't overfit to them):
  ...
  * `../tutor` and its plugins (`tutor-*`) — Tutor configures/runs Open edX. Treat as extended documentation; we're building something smaller and simpler, so don't copy Tutor patterns.
* Use `rg`, not `grep`.
